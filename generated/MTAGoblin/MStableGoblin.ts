...
export class MStableGoblin extends ethereum.SmartContract {
  static bind(address: Address): MStableGoblin {
    return new MStableGoblin("MStableGoblin", address);
  }
  ...
  mta(): Address {
    let result = super.call("mta", "mta():(address)", []);

    return result[0].toAddress();
  }

  try_mta(): ethereum.CallResult<Address> {
    let result = super.tryCall("mta", "mta():(address)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
